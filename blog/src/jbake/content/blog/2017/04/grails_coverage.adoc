...
IMPORTANT: All the code coverage mention in the entry is about the `api` module.

=== Initial configuration

Lets add both plugins to Gradle configuration. First declare plugins
in module `:api` in file `build.gradle`:

[source, groovy]
.build.gradle
----
include::{sources}/api/build.gradle[tags=cobertura_plugin, indent=0]
----
<1> Cobertura plugin
<2> Jacoco plugin

WARNING: You must declare the `plugins { }` block before the other
plugin declarations

NOTE: Jacoco is bundled with Gradle, and it has a default version. You
can change the Jacoco version. I'll show that later on.
...
| cobertura
| jacoco
a| image::2017/04/grails_coverage/cobertura_init.png[]
a| image::2017/04/grails_coverage/jacoco_init.png[]
|===
...
----
include::{sources}/api/build.gradle[tags=coverageExcludes, indent=0]
----
...
.Excluding classes
----
include::{sources}/api/build.gradle[tags=excludeClassesJacoco, indent=0]
----
...
| cobertura
| jacoco
a| image::2017/04/grails_coverage/cobertura_reduce_class_noise.png[]
a| image::2017/04/grails_coverage/jacoco_reduce_class_noise.png[]
|===
...
| cobertura
| jacoco
a| image::2017/04/grails_coverage/cobertura_greetings_first.png[]
a| image::2017/04/grails_coverage/jacoco_greetings_first.png[]
|===
